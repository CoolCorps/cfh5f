function newStormMaterial(gl, ext, texture) {
    const vsSource = `
...
// Mix final noise value
  ...
  float surface( vec3 coord ) {
    ...
    void main(void) {
        vec2 texcoord = vTexcoord * 7.5;

        float fogAmount = smoothstep(100.0, 200.0, length(fogPosition));

        float noise = surface(vec3(texcoord, time * 0.2));

        vec4 sandColor = vec4(201.0 / 255.0, 172.0 / 255.0, 130.0 / 255.0, 1);

        vec4 albedo = mix(vec4(fogColor.rgb * 0.5, fogColor.a), sandColor, noise);

        vec4 fragColor = mix(albedo, fogColor, fogAmount);
  ...
  `;
    ...
    const defaultMaterial = {
        ...
        uniformLocations: {
            ...
            time: gl.getUniformLocation(defaultMaterialProgram, 'time')
        },
        init: (draw) => {
        ...
        setUniforms: (draw, uniforms) => {
            ...
            gl.uniform1f(draw.material.uniformLocations.time, uniforms.time);
